...
void mplayer_defaultmenu(mplayer_t* mplayer) {
    ...
    while(SDL_PollEvent(&mplayer->e)) {
        if(mplayer->e.type == SDL_QUIT) {
            ...
            int x = setting_iconbtn.btn_canvas.x, y = setting_iconbtn.btn_canvas.y,
                w = setting_iconbtn.btn_canvas.w, h = setting_iconbtn.btn_canvas.h;
            if((mplayer->e.button.x <= x + w && mplayer->e.button.x >= x) &&
                (mplayer->e.button.y <= y + h && mplayer->e.button.y >= y)) {
            ...
            for(int i=0;i<tab_info_size;i++) {
                ...
                y = mplayer->e.button.y <= tab_info[i].text_canvas.y + tab_info[i].text_canvas.h + 10 &&
                       mplayer->e.button.y >= tab_info[i].text_canvas.y;
                if(x && y && TAB_INIT) {
                    if(!hover) {
                        mplayer_setcursor(mplayer, MPLAYER_CURSOR_POINTER);
                        hover = true;
            ...
            for(int i=0;i<tab_info_size;i++) {
                ...
                y = mplayer->e.button.y <= tab_info[i].text_canvas.y + tab_info[i].text_canvas.h + 10 &&
                       mplayer->e.button.y >= tab_info[i].text_canvas.y;
                if(x && y && TAB_INIT) {
                    if(!tab_info[i].active) {
                        for(int j=0;j<tab_info_size;j++) {
                            if(tab_info[j].active) {
                                tab_info[j].active = false;
                                break;
                            }
                        }
                        active_tab = tab_info[i].id;
                        tab_info[i].underline_color = underline_color;
                        tab_info[i].active = true;
                    }
                }
            }
            x = setting_iconbtn.btn_canvas.x, y = setting_iconbtn.btn_canvas.y;
            w = setting_iconbtn.btn_canvas.w, h = setting_iconbtn.btn_canvas.h;
            if((mplayer->e.button.x <= x + w && mplayer->e.button.x >= x) && (mplayer->e.button.y <= y + h && mplayer->e.button.y >= y)) {
...
void mplayer_settingmenu(mplayer_t* mplayer) {
    ...
    while(SDL_PollEvent(&mplayer->e)) {
        if(mplayer->e.type == SDL_QUIT) {
            ...
            int x = setting_btns[0].btn_canvas.x, y = setting_btns[0].btn_canvas.x,
                w = setting_btns[0].btn_canvas.w, h = setting_btns[0].btn_canvas.h;
            ...
            if((mplayer->e.button.x <= x + w && mplayer->e.button.x >= x) &&
                (mplayer->e.button.y <= y + h && mplayer->e.button.y >= y)) {
                    printf("You pressed the setting back button\n");
                    mplayer->menu_opt = MPLAYER_DEFAULT_MENU;
                    mplayer_setup_menu(mplayer);
                    mplayer_setcursor(mplayer, MPLAYER_CURSOR_DEFAULT);
            }
